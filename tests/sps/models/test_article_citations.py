...
class AuthorsTest(TestCase):
    ...
    def test_chapter_title(self):
        ...
        obtained = list(ArticleCitations(xml_tree).article_citations)[0].get("chapter_title")
    ...
    def test_part_title(self):
        ...
        xml = """
            ...
            """
        xml_tree = etree.fromstring(xml)
        obtained = list(ArticleCitations(xml_tree).article_citations)[0].get("part_title")
        expected = "Tópicos especiais em piscicultura de água doce tropical intensiva"
        self.assertEqual(obtained, expected)