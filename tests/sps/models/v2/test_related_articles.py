...
from packtools.sps.models.v2.related_articles import (
    RelatedArticle,
    RelatedArticlesParent,
    RelatedArticles,
)
...
class RelatedArticleTest(TestCase):
    def test_related_article(self):
        ...
        xml = """
            ...
            """
        obtained = RelatedArticle(
            xml_utils.get_xml_tree(xml).xpath(".//related-article")[0]
        ).data()
        expected = {
            "ext-link-type": "doi",
            "id": "A01",
            ...
            "full_tag": '<related-article ext-link-type="doi" id="A01" '
            'related-article-type="commentary-article" '
            'xlink:href="10.1590/0101-3173.2022.v45n1.p139">\n'
            "                Referência do artigo comentado: FREITAS, J. H. "
            "de. Cinismo e indiferenciación: la huella de Glucksmann en\n"
            "                <italic>El coraje de la verdad</italic>\n"
            "                de Foucault.\n"
            "                <bold>Trans/form/ação</bold>\n"
            "                : revista de Filosofia da Unesp, v. 45, n. 1, p. "
            "139-158, 2022.\n"
            "                </related-article>",
        }
...
class RelatedArticlesByNodeTest(TestCase):
    def test_related_articles(self):
        ...
        obtained = list(
            RelatedArticlesParent(
                xml_utils.get_xml_tree(xml).find(".")
            ).related_articles()
        )
        expected = [
            {
                ...
                'related-article-type="commentary-article" '
                'xlink:href="10.1590/0101-3173.2022.v45n1.p139">\n'
                "            Referência do artigo comentado: FREITAS, J. H. de. "
                "Cinismo e indiferenciación: la huella de Glucksmann en\n"
                "            <italic>El coraje de la verdad</italic>\n"
                "            de Foucault.\n"
                "            <bold>Trans/form/ação</bold>\n"
                "            : revista de Filosofia da Unesp, v. 45, n. 1, p. "
...
class RelatedArticlesTest(TestCase):
    def setUp(self):
        self.xml_tree = xml_utils.get_xml_tree(
            "tests/fixtures/htmlgenerator/related-article/varias_erratas.xml"
        )
    ...
    def test_related_articles_by_related_article_type(self):
        ...
        obtained = list(
            RelatedArticles(
                xml_utils.get_xml_tree(xml), related_article_type="commentary-article"
            ).related_articles()
        )
        expected = [
            {
