...
class TestDoiWithLang(unittest.TestCase):
    ...
    def test_data_missing_author_info(self):
        ...
        with patch('packtools.sps.models.article_doi_with_lang.XMLContribs') as mock_contribs:
            mock_contribs.return_value.contribs = [{'contrib_name': {}}]  # Missing required fields
            doi_with_lang = DoiWithLang(self.xmltree)
            data = doi_with_lang.data
            self.assertEqual(data[0]['authors'], [])  # Should handle missing author info gracefully
